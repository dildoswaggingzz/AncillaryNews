...
| `ImbalancePrice` | (already ingested) — additionally exposes `SpotPriceEUR`, `mFRRMarginalPriceUpEUR/DKK`, `mFRRMarginalPriceDownEUR/DKK` | **Resolved 2026-07-17, both false leads:** `mFRRMarginalPriceUp/DownEUR` matches `MfrrEnergyActivationMarket.mFRRSAUp/DownEUR` value-for-value across every sample checked — same clearing price surfaced in two datasets, not new signal (also a nice cross-dataset sanity check that the M0-addendum mFRR EAM ingestion is correct). `SpotPriceEUR` matches `DayAheadPrices.DayAheadPriceEUR` exactly at matching timestamps (verified 05:45 → 150.960007/150.96, 06:00 → 175.899994/175.9) — already ingested under a different dataset name. Neither field needs separate ingestion. |
